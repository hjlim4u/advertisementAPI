...
public abstract class ExternalAPIPolicy implements PolicyState {
    ...
    public List<Advertisement> transmit(List<Advertisement> advertisements, long userId, int total) {
        ...
        List<Integer> externalResult = externalAPIService.getAdCampaignIds(userId, ad_ids, total);
        return postProcess(externalResult, advertisements, userId, total);
    }

    abstract List<Advertisement> postProcess(List<Integer> externalResult, List<Advertisement> advertisements, long userId, int total);
}
