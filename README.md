...
# 광고 추천 로직 구현하기
광고 데이터는 src/main/resources/ad_campaigns.json 파일에서 확인할 수 있습니다. 데이터베이스에 주어진 파일을 임포트 하여 사용합니다. 파일에는 다음과 같은 정보가 담겨있습니다.
...
| Advertisement Service | 도메인 객체 기반 비즈니스 로직을 구현해놓은 서비스 객체(AdvertisementService) |
...
| External System Adapter | 주어진 url parameter 값에 따라 pctr값을 보내주는 외부 API(RestTemplate)  |
| Input Port(External) | RestTemplate로 받아온 Json file을 Object file로 변환시키기 위해 필요한 객체(Response) |
...
| AdvertisementService | @Order로 정렬된 List<PolicyState>에서 userId % 4 로 송출정책 선택 |

확장성을 고려하여 External API Policy는 외부API에 요청하여 PCTR값이 높은 광고의 id값을 가져오는 것까지 수행하고 나머지 후처리는 템플릿 메서드 패턴을 활용하여 자식 클래스에 구현을 맡긴다.<br/>
광고 송출에 필요한 정보(광고 목록, userId, 송출 개수)는 PolicyState.transmit() 파라미터로 전달됩니다.<br/>
송출 정책 선택은 PolicyState 인터페이스에만 의존하므로 PolicyState 구현체를 추가하는 것만으로 정책 확장이 가능합니다.<br/>

## 광고 송출 로직 변경
1. PolicyState나 ExternalPolicy를 구현한 광고 송출 정책 추가
2. 새 정책 클래스에 @Component와 @Order(n) 부여 (n = userId % N 에 대응)
3. AdvertisementService의 List<PolicyState>에 자동 주입되며, userId % policies.size() 인덱스로 선택됩니다.<br/>
...
#### 2. RestTemplate<br/>
외부 pCTR API 호출은 결과를 즉시 사용하므로 동기 RestTemplate 사용 (초기에는 WebClient를 썼으나 모든 호출부가 block()하여 제거)<br/>
<br/>
